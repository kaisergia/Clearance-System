...
export default function DepartmentLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { label: "Dashboard", href: "/department/dashboard", icon: "dashboard" },
    { label: "Constituents", href: "/department/constituents", icon: "person_search" },
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-surface-container-lowest border-b border-outline-variant flex items-center justify-between px-4 z-40 md:hidden">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-secondary uppercase tracking-wider mt-0.5">Department Portal</p>
          </div>
        </div>
      ...
      >
        <nav className="px-4 space-y-1">
          ...
          <div className="pt-4 mt-2 border-t border-outline-variant px-4 flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-700">Department Staff</span>
            <button
