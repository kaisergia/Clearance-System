...
export default function DepartmentDashboard() {
  ...
  useEffect(() => {
    // 1. Get the logged in department
    const departmentId = localStorage.getItem("departmentId");
    if (departmentId) {
      const department = mockDepartments.find((o) => o.id === Number(departmentId));
    ...
    if (departmentId) {
      const department = mockDepartments.find((o) => o.id === Number(departmentId));
      const deptStudents = mockStudents.filter(s => department ? s.department === department.abbreviation : true);
              ...
              {mockStudents
                ...
                return (
                  <tr key={student.id} className={`hover:bg-surface-container-low/20 transition-all duration-150 ${status === "Rejected" ? "bg-[#FEF2F2]/10" : ""}`}>
                    ...
                    <td className="py-4 px-6 text-center">
                      <Link
                        href={`/department/constituents/${student.id}`}
