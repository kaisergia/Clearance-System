...
export default function ClearanceRequirementsPage() {
  ...
  useEffect(() => {
    ...
    if (storedOfficeId) {
      ...
      clearanceService.getOfficeById(oid).then(office => {
        if (office) setActiveOffice(office);
      });

      // DATABASE SWAP POINT: replace with DB query via clearanceService
      clearanceService.getOfficeRequirements(oid).then(setRequirements);

      // Check access dynamically based on published clearance flows
      fetch(`/api/requirements/check-access?type=office&id=${oid}`)
        .then((res) => res.json())
        .then((data) => {
        ...
        .catch((err) => {
          console.error("Error checking signatory access:", err);
          setHasAccess(true);
        });
    }
  }, []);
  ...
  const saveRequirements = (updatedReqs: Requirement[]) => {
    if (officeId) {
      // DATABASE SWAP POINT: clearanceService.saveOfficeRequirements wraps the localStorage write.
      clearanceService.saveOfficeRequirements(officeId, updatedReqs);
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      {activeTab === "evaluator" ? (
        <RequirementBatchEvaluator
          ...
          onRefresh={() => {
            if (officeId) clearanceService.getOfficeRequirements(officeId).then(setRequirements);
          }}
        />
      ) : (
