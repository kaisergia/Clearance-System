...
export default function ConstituentsPage() {
  ...
  const loadData = async () => {
    const rawOfficeId = localStorage.getItem("officeId");
    let targetOfficeId = rawOfficeId ? Number(rawOfficeId) : null;
    let currentOffice = null;

    if (targetOfficeId) {
      currentOffice = await clearanceService.getOfficeById(targetOfficeId);
    }
    ...
    if (currentOffice) {
      setActiveOffice(currentOffice);
      setCurrentOfficeId(targetOfficeId);
    }

    const officeReqs = targetOfficeId ? await clearanceService.getOfficeRequirements(targetOfficeId) : [];
    ...
    for (const student of allStudents) {
      ...
      if (!hasRequirements) {
        computedStatus = (officeRec?.status as any) || "Pending";
      } else {
        const completedTasksIdx = Array.isArray(officeRec?.completedTasks) ? officeRec.completedTasks : [];
        ...
        else if (hasRejected || officeRec?.status === "Rejected") computedStatus = "Rejected";
        else if (hasPendingReview || officeRec?.status === "Submitted") computedStatus = "Submitted";
