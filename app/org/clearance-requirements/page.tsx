...
export default function OrgClearanceRequirementsPage() {
  ...
  const saveRequirements = (updatedReqs: Requirement[]) => {
    if (org) {
      // DATABASE SWAP POINT: clearanceService.saveOrgRequirements wraps the localStorage write.
      clearanceService.saveOrgRequirements(org.id, updatedReqs);
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      {activeTab === "evaluator" ? (
        <RequirementBatchEvaluator
          ...
          onRefresh={() => {
            if (org?.id) clearanceService.getOrgRequirements(org.id).then(setRequirements);
          }}
        />
      ) : (
